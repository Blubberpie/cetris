...
namespace constants {
	...
	inline constexpr int DROP_TICK_DELAY = 700; // 1 tile per ? ms
	inline constexpr int DROP_DELAY = 60; // How long a key should be held before repetition is triggered
	inline constexpr int DROP_REPEAT_RATE = 17; // How fast to drop when key is held
	inline constexpr int DELAYED_AUTO_SHIFT = 133; // How long a key should be held before repetition is triggered
	inline constexpr int AUTO_REPEAT_RATE = 17; // How fast a block moves left/right
	inline constexpr int LOCK_DELAY = 500;
}